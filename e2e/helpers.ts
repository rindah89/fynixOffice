 ...
 * userData dir (via FYNIXOFFICE_USER_DATA) so runs never touch real settings
...
interface LaunchOptions {
  ...
  userDataDir?: string
  /** UI language override (FYNIXOFFICE_LANG); defaults to English for stable assertions */
  lang?: string
...
export async function launchShell(options: LaunchOptions): Promise<LaunchedApp> {
  ...
  const userDataDir = options.userDataDir ?? (await mkdtemp(join(tmpdir(), 'fynixoffice-e2e-')))
  ...
  const app = await electron.launch({
    ...
    env: {
      ...hostEnv,
      FYNIXOFFICE_USER_DATA: userDataDir,
      FYNIXOFFICE_LANG: options.lang ?? 'en',
