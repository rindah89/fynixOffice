...
Set `OFFICE_PRIVACY_SUBJECT` to the requester's canonical opaque UUID, `OFFICE_PRIVACY_IDENTITY_EVIDENCE_REF` to a controlled `urn:fynix:` or `evidence://` reference, and `OFFICE_PRIVACY_OUTPUT` to a new protected path. The command never overwrites an existing export.

For a verified erasure request, configure the same complete `OFFICE_USER_DATA_DIRS`, set `OFFICE_PRIVACY_LAWFUL_BASIS`, and optionally set `OFFICE_PRIVACY_HOLD_FILE` to a protected JSON array of canonical subject UUIDs under active preservation. Run `npm run privacy:erase -w @fynixoffice/governance`. The command opens the CyberAudit request, validates every surface before mutation, blocks before mutation on a legal hold, moves all Office-controlled settings, recent-file metadata, project metadata and local AI chats out of their live locations with rollback on staging or receipt failure, and closes the request with a SHA-256-bound receipt. The receipt is mode 0600 and records the exact affected paths. User document, workbook, presentation and PDF contents are deliberately retained because their owning workspace records policy controls classification, holds and disposition; never describe those retained records as erased. DG-03 remains partial until correction, restriction and objection are executable across the same declared surfaces.
