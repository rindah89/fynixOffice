  ...
  const runAi = (action: 'summarize' | 'rewrite' | 'polish' | 'format') =>
    run(() => {
      ...
      const instruction = {
        ...
        rewrite:
          `Rewrite the user-highlighted section while preserving its meaning. You must call replace_selection with expectedText set to the exact selected text, from=${selectedRange.from}, to=${selectedRange.to}, and your rewrite as replacementText. Do not call replace_blocks. Leave all surrounding content unchanged.`,
        polish:
