...
const localAlias = {
  '@fynixoffice/docx-engine': resolve(__dirname, '../../packages/docx-engine/src/index.ts'),
}
...
export default defineConfig({
  ...
  // @fynixoffice/* deps ship as raw TS source with extensionless imports, so they
  // must be bundled — externalizing them yields ERR_MODULE_NOT_FOUND under Node
  // (same setup as apps/slides).
  main: {
    plugins: [externalizeDepsPlugin({ exclude: ['@fynixoffice/electron-utils'] })],
  ...
  renderer: {
    ...
    resolve: { alias: localAlias },
    server: {
      // Overridable so multiple fynixoffice dev instances can coexist (default 5173).
